...
export const templateInfoText = `<div style=" padding: 10px 10px; background-color: rgba(250, 250, 250, 0.8); color: rgba(20, 20, 20, 1); border-radius: 4px;" >
  <div style="font-size: 17px; font-weight: 450;">
    <!--ADD TEXT HERE-->
  </div>
</div>`;
...
// =================================================================
// FIRESTORE CONNECTION WARMUP
// =================================================================

// Warms up the Firestore connection to avoid cold start delay on first write
export const warmupFirestore = async (postId) => {
  if (!postId) return;
  try {
    // Do a write operation to fully warm up the connection
    // We update a post's order_index to its current value (no-op but establishes connection)
    const postRef = doc(db, "posts", postId);
    const postSnap = await getDoc(postRef);
    if (postSnap.exists()) {
      const currentIndex = postSnap.data().order_index;
      await updateDoc(postRef, { order_index: currentIndex });
    }
  } catch {
    // Ignore errors - we just want to open the connection
  }
};

// One-time cleanup: removes _lastAccess field from all pages (run once then delete)
export const cleanupLastAccessField = async () => {
  try {
    const pagesSnap = await getDocs(collection(db, "pages"));
    const batch = writeBatch(db);
    let count = 0;
    pagesSnap.forEach((pageDoc) => {
      if (pageDoc.data()._lastAccess) {
        batch.update(pageDoc.ref, { _lastAccess: deleteField() });
        count++;
      }
    });
    if (count > 0) {
      await batch.commit();
      console.log(`Cleaned up _lastAccess from ${count} pages`);
    }
  } catch (error) {
...
export const uploadFile = async (file, path, maxSizeBytes = null) => {
  if (!file) {
...
export const getPages = async (userId, isOwner = false) => {
  ...
  return querySnapshot.docs.map((doc) => {
    const { _lastAccess, ...data } = doc.data();
    return {
      id: doc.id,
      ...data,
...
export const getPageById = async (pageId) => {
  ...
  if (docSnap.exists()) {
    const { _lastAccess, ...data } = docSnap.data();
    return {
      id: docSnap.id,
      ...data,
      created_date: data.created_date.toDate(),
...
export const createPage = async (pageData, userId) => {
  ...
  const docRef = await addDoc(pagesCollectionRef, {
    ...
    pageMetaData: {
      infoText1: templateInfoText,
      infoText2: templateInfoText,
    },
    postCount: 0,
  });
...
export const deletePage = async (pageData) => {
  ...
  if (!pageData.id || pageData.id.startsWith("temp-")) {
    console.warn(
      "[deletePage] Skipping delete for temp/missing ID:",
      pageData.id,
    );
    return;
  }
  ...
  if (!pageDoc.exists()) {
    ...
    console.warn(
      "[deletePage] Page not found, may have been already deleted:",
      pageData.id,
    );
    return;
  }
...
// Atomic swap of order_index between two posts (used for arrow reordering)
export const swapPostOrder = async (
  postAId,
  postANewIndex,
  postBId,
  postBNewIndex,
  ...
  const batch = writeBatch(db);
  batch.update(doc(db, "posts", postAId), { order_index: postANewIndex });
  batch.update(doc(db, "posts", postBId), { order_index: postBNewIndex });
  await batch.commit();
};
...
export const updatePost = async (postId, postData, allPosts) => {
  ...
  /* ---------------------------------------------
   * 2. CLEANUP CHECK (R2 FILE REPLACEMENT/REMOVAL)
   * ------------------------------------------- */
  // Case A: File-to-file replacement (content URL changed)
  if (
    originalPost.content_type === "file" &&
    originalPost.content &&
    postData.content_type === "file" &&
    postData.content &&
  ...
  // Case B: Switching FROM file type to another type (photo-only or url)
  // This prevents orphaned files in R2
  // Note: Only trigger if content_type is explicitly provided (not undefined)
  if (
    originalPost.content_type === "file" &&
    originalPost.content &&
    postData.content_type !== undefined &&
    postData.content_type !== "file"
  ) {
    console.log("Switching from file type: deleting old R2 object...");
    await deleteFileViaAPI(originalPost.content);
    // Clear the content field since it's no longer a file URL
    if (postData.content_type === "photo-only") {
      postData.content = "";
    }
  }
  ...
  const {
    ...
    clientId,
    isUploadingHeic,
    contentFile, // Client-only: File object for new uploads
    originalContentType, // Client-only: for cleanup detection
    originalContent, // Client-only: for cleanup detection
    ...cleanPostData
  } = postData;
...
export const deletePost = async (postData) => {
  ...
  if (!postData.id || postData.id.startsWith("temp-")) {
    console.warn(
      "[deletePost] Skipping delete for temp/missing ID:",
      postData.id,
    );
    return;
  }
  ...
  if (!postDoc.exists()) {
    ...
    console.warn(
      "[deletePost] Post not found, may have been already deleted:",
      postData.id,
    );
    return;
  }
...
export const getPageBySlug = async (userId, slug) => {
  ...
  const { _lastAccess, ...data } = pageDoc.data();
  return {
    id: pageDoc.id,
    ...data,
    created_date: data.created_date.toDate(),
